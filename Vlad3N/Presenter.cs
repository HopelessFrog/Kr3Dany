...
namespace Kr3G
{
    ...
    public class Presenter:IPresenter
    {
        ...
        const int EXPECTED_AMOUNT_DATA = 5;
        ...
        private void MainForm_OpenFile(object? sender, EventArgs e)
        {
            ...
            for (int i = 0; i < fileValue.Length; i++)
            {
                ...
                Thread.Sleep(500);
                mainForm.C = chartLine[3];
                Thread.Sleep(500);
                mainForm.A = chartLine[4];
                Draw();
            }
        ...
        private void Draw()
        {
            ...
            string errors = validator.PreValidation(new DataHolder<string>(mainForm.LeftBorder, mainForm.RightBorder, mainForm.Step, mainForm.C,mainForm.A));
            if (errors != "")
            {
                messageService.ShowError(errors);
                return;
            }
            dataHolders.Add(new DataHolder<double>(double.Parse(mainForm.LeftBorder), double.Parse(mainForm.RightBorder), double.Parse(mainForm.Step), double.Parse(mainForm.C), double.Parse(mainForm.A)));
            ...
            string? label = "[" + dataHolders[dataHolders.Count - 1].LeftBorder.ToString() + "," + dataHolders[dataHolders.Count - 1].RightBorder.ToString() + ","
                + dataHolders[dataHolders.Count - 1].Step.ToString() + "," + dataHolders[dataHolders.Count - 1].C.ToString() + "," + dataHolders[dataHolders.Count - 1].A.ToString() + "]";
